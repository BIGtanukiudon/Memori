...
import { rowToWorkLog, type WorkLogRow } from "./mappers";
...
export async function createWorkLog(db: Db, input: CreateWorkLogInput): Promise<WorkLog> {
  const body = requireBody(input.body);

  const rows = await db.select<
    { project_id: string; task_title: string; project_name: string }[]
  >(
    `SELECT t.project_id AS project_id, t.title AS task_title, p.name AS project_name
  ...
  await db.execute(
    ...
    [id, input.taskId, task.project_id, body, task.task_title, task.project_name, ts, ts],
  );
...
export async function listWorkLogsByTask(db: Db, taskId: string): Promise<WorkLog[]> {
  const rows = await db.select<WorkLogRow[]>(
    `SELECT id, task_id, project_id, body, task_title, project_name, created_at, updated_at
     FROM work_logs WHERE task_id = ? ORDER BY created_at DESC`,
    [taskId],
  );
  return rows.map(rowToWorkLog);
}

export async function updateWorkLog(db: Db, id: string, body: string): Promise<void> {
