...
vi.mock("@/db/columns", () => ({
    listColumns: (...a) => listColumns(...a),
}));
vi.mock("@/db/tasks", () => ({
    searchTasksForLog: (...a) => searchTasksForLog(...a),
}));
vi.mock("@/db/workLogs", () => ({
    getLastLoggedTaskId: (...a) => getLastLoggedTaskId(...a),
}));
...
vi.mock("@/lib/events", async () => {
    const actual = await vi.importActual("@/lib/events");
    return {
        ...actual,
        emitTaskCreated: (...a) => emitTaskCreated(...a),
        emitWorkLogAdded: (...a) => emitWorkLogAdded(...a),
        listenQuickModeChanged: (...a) => listenQuickModeChanged(...a),
    };
});
vi.mock("@/lib/window", () => ({
    hideQuickWindow: (...a) => hideQuickWindow(...a),
    isTauriRuntime: (...a) => isTauriRuntime(...a),
}));
...
describe("QuickApp", () => {
    ...
    it("登録: createTaskAction → emitTaskCreated → hideQuickWindow の順で呼ばれる", async () => {
        ...
        expect(createTaskAction).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            projectId: "P1",
            columnId: "C1",
            title: "新タスク",
    ...
    it("登録後にフォーム状態がリセットされる", async () => {
        ...
        // 登録後にフォームがリセットされる
        await waitFor(() => {
            const t = screen.getByLabelText("タスク名");
            expect(t.value).toBe("");
        });
    });
...
describe("QuickApp - ログモード", () => {
    ...
    it("既定タスクの解決: getLastLoggedTaskIdの結果を対象タスク欄に表示する", async () => {
        ...
        await waitFor(() => expect(captured).not.toBeNull());
        captured({ mode: "log" });
        await waitFor(() => expect(screen.getByLabelText("対象タスク")).toHaveValue("開発 / 実装する"));
    });
