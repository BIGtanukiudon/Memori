...
describe("applyTaskSort", () => {
  const dueSort: TaskSort = { key: "due", asc: true };
  const prioritySort: TaskSort = { key: "priority", asc: false };
  const updatedSort: TaskSort = { key: "updated", asc: false };
  const createdSort: TaskSort = { key: "created", asc: true };
...
describe("DueFilter type", () => {
  it("4つの値が許可される", () => {
    const values: DueFilter[] = ["all", "today", "overdue", "none"];
