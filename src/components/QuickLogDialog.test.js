...
describe("QuickLogDialog", () => {
    it("既定タスクがある場合: 対象タスク欄に「プロジェクト名 / タスク名」が入り、本文欄にフォーカス", async () => {
        render(_jsx(QuickLogDialog, { initialTask: taskA, searchTasks: vi.fn().mockResolvedValue([]), onSubmit: () => { }, onCancel: () => { } }));
    ...
    it("既定タスクがない場合: 対象タスク欄にフォーカス", async () => {
        ...
        render(_jsx(QuickLogDialog, { initialTask: null, searchTasks: searchTasks, onSubmit: () => { }, onCancel: () => { } }));
    ...
    it("検索で対象タスクを切り替えられる", async () => {
        ...
        render(_jsx(QuickLogDialog, { initialTask: taskA, searchTasks: searchTasks, onSubmit: () => { }, onCancel: () => { } }));
    ...
    it("Ctrl+Enterで onSubmit が呼ばれる", async () => {
        ...
        render(_jsx(QuickLogDialog, { initialTask: taskA, searchTasks: vi.fn().mockResolvedValue([]), onSubmit: onSubmit, onCancel: () => { } }));
    ...
    it("Escで onCancel が呼ばれる", async () => {
        ...
        render(_jsx(QuickLogDialog, { initialTask: taskA, searchTasks: vi.fn().mockResolvedValue([]), onSubmit: () => { }, onCancel: onCancel }));
    ...
    it("対象タスク未選択では追加ボタンが無効", async () => {
        const user = userEvent.setup();
        render(_jsx(QuickLogDialog, { initialTask: null, searchTasks: vi.fn().mockResolvedValue([]), onSubmit: () => { }, onCancel: () => { } }));
        const body = screen.getByLabelText("本文");
