...
const column: Column = { id: "C0", projectId: "P", name: "Todo", position: 0 };
...
describe("KanbanColumn", () => {
  ...
  it("タスク件数が表示される", () => {
    render(<KanbanColumn column={column} tasks={[task("T1", "a"), task("T2", "b")]} />);
    expect(screen.getByText("2")).toBeInTheDocument();
  });

  it("配下タスクのタイトルが順に表示される", () => {
    render(<KanbanColumn column={column} tasks={[task("T1", "あ"), task("T2", "い")]} />);
  ...
  it("編集ボタン → インライン入力 → Enterで onRename が呼ばれる", async () => {
    ...
    render(<KanbanColumn column={column} tasks={[]} onRename={onRename} />);
