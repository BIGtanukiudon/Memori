...
describe("KanbanColumn", () => {
    ...
    it("「完了列に設定」ボタンで onSetDoneColumn が呼ばれる", async () => {
        ...
        render(_jsx(KanbanColumn, { column: column, tasks: [], isDoneColumn: false, onSetDoneColumn: onSetDoneColumn }));
    ...
    it("完了列の場合は「完了列を解除」ボタンが表示される", async () => {
        const onSetDoneColumn = vi.fn();
        render(_jsx(KanbanColumn, { column: column, tasks: [], isDoneColumn: true, onSetDoneColumn: onSetDoneColumn }));
