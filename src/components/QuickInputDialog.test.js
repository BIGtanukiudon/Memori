...
const loadColumns = async (projectId) => columnsByProject[projectId] ?? [];
describe("QuickInputDialog", () => {
    it("初期表示: 最初のプロジェクトとその先頭列が選択され、タイトル入力にフォーカス", async () => {
        render(_jsx(QuickInputDialog, { projects: projects, loadColumns: loadColumns, onSubmit: () => { }, onCancel: () => { } }));
    ...
    it("Enterで onSubmit が現在の入力で呼ばれる", async () => {
        ...
        render(_jsx(QuickInputDialog, { projects: projects, loadColumns: loadColumns, onSubmit: onSubmit, onCancel: () => { } }));
        await screen.findByDisplayValue("Todo");
    ...
    it("Escで onCancel が呼ばれる", async () => {
        ...
        render(_jsx(QuickInputDialog, { projects: projects, loadColumns: loadColumns, onSubmit: () => { }, onCancel: onCancel }));
        await screen.findByDisplayValue("Todo");
    ...
    it("登録ボタンでもonSubmitが呼ばれる", async () => {
        ...
        expect(onSubmit).toHaveBeenCalledWith(expect.objectContaining({ title: "X", projectId: "P1", columnId: "C1" }));
    });
    it("プロジェクトが0件のとき案内文を表示し、Enterは無効", async () => {
        const user = userEvent.setup();
        const onSubmit = vi.fn();
        render(_jsx(QuickInputDialog, { projects: [], loadColumns: loadColumns, onSubmit: onSubmit, onCancel: () => { } }));
