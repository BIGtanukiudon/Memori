...
describe("DeleteColumnDialog", () => {
    it("列名とタスク件数を表示する", () => {
        render(_jsx(DeleteColumnDialog, { open: true, column: sourceColumn, otherColumns: otherColumns, taskCount: 3, onCascade: () => { }, onMoveAndDelete: () => { }, onCancel: () => { } }));
        expect(screen.getByText(/Todo/)).toBeInTheDocument();
        expect(screen.getByText(/3/)).toBeInTheDocument();
    });
    it("「まとめて削除」でonCascadeが呼ばれる", async () => {
        const onCascade = vi.fn();
        render(_jsx(DeleteColumnDialog, { open: true, column: sourceColumn, otherColumns: otherColumns, taskCount: 3, onCascade: onCascade, onMoveAndDelete: () => { }, onCancel: () => { } }));
    ...
    it("移動先列を選択して「移動して削除」でonMoveAndDeleteが呼ばれる", async () => {
        ...
        render(_jsx(DeleteColumnDialog, { open: true, column: sourceColumn, otherColumns: otherColumns, taskCount: 3, onCascade: () => { }, onMoveAndDelete: onMoveAndDelete, onCancel: () => { } }));
    ...
    it("他列が無いときは「移動して削除」ボタンが無効", () => {
        render(_jsx(DeleteColumnDialog, { open: true, column: sourceColumn, otherColumns: [], taskCount: 3, onCascade: () => { }, onMoveAndDelete: () => { }, onCancel: () => { } }));
        expect(screen.getByRole("button", { name: /移動して削除/ })).toBeDisabled();
    });
    it("キャンセルでonCancelが呼ばれる", async () => {
        const onCancel = vi.fn();
        render(_jsx(DeleteColumnDialog, { open: true, column: sourceColumn, otherColumns: otherColumns, taskCount: 3, onCascade: () => { }, onMoveAndDelete: () => { }, onCancel: onCancel }));
