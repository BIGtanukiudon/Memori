...
describe("TaskDetailDialog", () => {
    it("既存タスクの値を初期表示する", () => {
        render(_jsx(TaskDetailDialog, { open: true, task: baseTask, onSave: () => { }, onDelete: () => { }, onClose: () => { } }));
    ...
    it("保存ボタンで編集内容を onSave に渡す", async () => {
        ...
        const onSave = vi.fn();
        render(_jsx(TaskDetailDialog, { open: true, task: baseTask, onSave: onSave, onDelete: () => { }, onClose: () => { } }));
    ...
    it("削除ボタンで確認後 onDelete が呼ばれる", async () => {
        ...
        render(_jsx(TaskDetailDialog, { open: true, task: baseTask, onSave: () => { }, onDelete: onDelete, onClose: () => { } }));
    ...
    it("キャンセルボタンで onClose が呼ばれる", async () => {
        const user = userEvent.setup();
        const onClose = vi.fn();
        render(_jsx(TaskDetailDialog, { open: true, task: baseTask, onSave: () => { }, onDelete: () => { }, onClose: onClose }));
        await user.click(screen.getByRole("button", { name: "閉じる" }));
        expect(onClose).toHaveBeenCalled();
    });
    it("task が null の場合は何も描画しない", () => {
        render(_jsx(TaskDetailDialog, { open: true, task: null, onSave: () => { }, onDelete: () => { }, onClose: () => { } }));
